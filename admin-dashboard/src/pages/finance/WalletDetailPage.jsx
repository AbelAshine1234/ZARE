...
const WalletDetailPage = () => {
  ...
  useEffect(() => {
    if (id) {
      console.log('[WalletDetailPage] URL id param:', id);
  ...
  useEffect(() => {
    // Per requirement: treat vendorId as the ID to load with; if vendor not yet loaded, use URL id
    const vendorIdToUse = vendor?.id || null;
    const resolvedId = vendorIdToUse ?? (id ? Number(id) : null);
    console.log('[WalletDetailPage] Vendor loaded?', !!vendor, 'vendorId:', vendorIdToUse, 'url id:', id, 'resolvedId (for transactions):', resolvedId);
    if (resolvedId) {
      console.log('[WalletDetailPage] Fetching wallet/transactions with ID:', resolvedId);
      dispatch(fetchWalletByUserId(resolvedId));
      dispatch(fetchWalletTransactions({ userId: resolvedId, page: currentPage, limit: 20 }));
    }
  }, [dispatch, vendor?.id, id, currentPage]);
  ...
  const handleRefresh = () => {
    const vendorIdToUse = vendor?.id || null;
    const resolvedId = vendorIdToUse ?? (id ? Number(id) : null);
    console.log('[WalletDetailPage] Refresh clicked. vendorId:', vendorIdToUse, 'url id:', id, 'resolvedId:', resolvedId);
    if (resolvedId) {
      console.log('[WalletDetailPage] Refresh fetching with ID:', resolvedId);
      dispatch(fetchWalletByUserId(resolvedId));
      dispatch(fetchWalletTransactions({ userId: resolvedId, page: currentPage, limit: 20 }));
    }
  };
  ...
  const exportToCSV = async () => {
    const vendorIdToUse = vendor?.id || null;
    const resolvedId = vendorIdToUse ?? (id ? Number(id) : null);
    console.log('[WalletDetailPage] Export CSV. vendorId:', vendorIdToUse, 'url id:', id, 'resolvedId:', resolvedId);
    if (!resolvedId) {
    ...
    try {
      console.log('[WalletDetailPage] Export CSV dispatch with ID:', resolvedId);
      const res = await dispatch(exportTransactionsCSV(resolvedId));
      if (exportTransactionsCSV.fulfilled.match(res)) {
        ...
        link.href = url;
        link.download = `wallet-transactions-${vendor?.name || 'vendor'}-${new Date().toISOString().split('T')[0]}.csv`;
